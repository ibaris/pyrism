...
class TMatrixSingle(Angles, object):
    ...
    @property
    def factor(self):
        return complex(0, 2 * PI * self.__N) / self.k0
    ...
    def __NMAX(self):
        ...
        if self.__radius_type == 2:
            ...
            for i, item in enumerate(self.__radius):
                radius[i] = equal_volume_from_maximum_wrapper(item, self.axis_ratio[i], self.shape_volume)
        ...
        nmax = NMAX_VEC_WRAPPER(radius=radius, radius_type=radius_type, wavelength=self.wavelength,
                                ...
                                axis_ratio=self.axis_ratio, shape=self.shape_volume, verbose=self.verbose)
